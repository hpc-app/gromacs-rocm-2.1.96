                    ...
                    )
{
    ...
    if (order == 4)
    {
        if (forceTreatment == PmeForceOutputHandling::Set)
        {
            hipLaunchKernelGGL((pme_gather_kernel<4, true, wrapX, wrapY>), dim3(dimGrid), dim3(dimBlock), 0, stream, *kernelParamsPtr);
        }
        else
        {
            hipLaunchKernelGGL((pme_gather_kernel<4, false, wrapX, wrapY>), dim3(dimGrid), dim3(dimBlock), 0, stream, *kernelParamsPtr);
        }
    }
    else
    {
