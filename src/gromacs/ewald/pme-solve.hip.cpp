                   ...
                   GridOrdering gridOrdering, bool computeEnergyAndVirial)
{
    ...
    if (gridOrdering == GridOrdering::YZX)
    {
        if (computeEnergyAndVirial)
        {
            hipLaunchKernelGGL((pme_solve_kernel<GridOrderingInternal::YZX, true>), dim3(blocks), dim3(threads), 0, stream, *kernelParamsPtr);
        }
        else
        {
            hipLaunchKernelGGL((pme_solve_kernel<GridOrderingInternal::YZX, false>), dim3(blocks), dim3(threads), 0, stream, *kernelParamsPtr);
        }
    }
    else if (gridOrdering == GridOrdering::XYZ)
    {
        if (computeEnergyAndVirial)
        {
            hipLaunchKernelGGL((pme_solve_kernel<GridOrderingInternal::XYZ, true>), dim3(blocks), dim3(threads), 0, stream, *kernelParamsPtr);
        }
        else
        {
            hipLaunchKernelGGL((pme_solve_kernel<GridOrderingInternal::XYZ, false>), dim3(blocks), dim3(threads), 0, stream, *kernelParamsPtr);
        }
    }
