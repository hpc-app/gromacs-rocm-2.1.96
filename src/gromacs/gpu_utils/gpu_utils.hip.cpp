...
{
    const hipDeviceProp_t *prop                        = &cuda_dev->prop;
...
bool canDetectGpus(std::string *errorMessage)
{
    ...
    GMX_ASSERT(stat != hipErrorInvalidValue, "An impossible null pointer was passed to hipDriverGetVersion");
        ...
        else
        {
            ...
            if ((stat = hipGetLastError()) != hipSuccess)
            {
                gmx_warning(gmx::formatString("An error occurred while sanity checking device #%d; %s: %s",
                                              devs[i].id, hipGetErrorName(stat), hipGetErrorString(stat)).c_str());
            }
        }
    }
