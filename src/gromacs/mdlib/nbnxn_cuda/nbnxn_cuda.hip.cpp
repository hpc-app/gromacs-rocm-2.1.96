...
texture<float, 1, hipReadModeElementType> nbfp_texref;

/*! Texture reference for LJ-PME parameters; bound to cu_nbparam_t.nbfp_comb */
texture<float, 1, hipReadModeElementType> nbfp_comb_texref;

/*! Texture reference for Ewald coulomb force table; bound to cu_nbparam_t.coulomb_tab */
texture<float, 1, hipReadModeElementType> coulomb_tab_texref;
...
static const bool bUseCudaLaunchKernel = false;
...
//        cudaLaunchKernel((void *)nb_kernel, dim_grid, dim_block, kernel_args, shmem, stream);
        hipLaunchKernelGGL((nb_kernel), dim3(dim_grid), dim3(dim_block), shmem, stream, *adat, *nbp, *plist, bCalcFshift);
#endif
    }
    else
    {
                                       ...
                                       int                     numParts)
{
    ...
    else
    {
        if (plist->haveFreshList)
        {
            hipLaunchKernelGGL((nbnxn_kernel_prune_cuda<true>), dim3(dim_grid), dim3(dim_block), shmem, stream, *adat, *nbp, *plist, numParts, part);
        }
        else
        {
            hipLaunchKernelGGL((nbnxn_kernel_prune_cuda<false>), dim3(dim_grid), dim3(dim_block), shmem, stream, *adat, *nbp, *plist, numParts, part);
        }
    }
